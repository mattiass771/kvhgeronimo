...
class GalleryEditModal extends Component {
    constructor() {
        super()
        this.state = {
            popup: true,
    ...
    componentDidMount() {
        axios.get(`https://kvhgeronimo.herokuapp.com/gallery/${this.props.itemID}`)
            .then(res => {
                this.setState({ name: res.data.name, date: res.data.date, showLink: res.data.showLink, about: res.data.about, links: res.data.links })
            })
            .catch(err => {
                console.log(err)
            });
    ...
    addGallery = () => {
        axios.post(`https://kvhgeronimo.herokuapp.com/gallery/update-gallery/${this.props.itemID}`, { 
            ...
            <Modal size="lg" show={this.state.popup} onHide={this.handleOnHide}>
                ...
                <Modal.Body style={{ whiteSpace: "pre-line" }}>
                    <Container>
                        ...
                        <Button variant="dark" style={{ width: "100%" }} onClick={this.addGallery}>Update Gallery</Button>
                    </Container>
                </Modal.Body>
            </Modal>
        )
