...
class LibraryEditModal extends Component {
    constructor() {
        super()
        this.state = {
            popup: true,
    ...
    componentDidMount() {
        axios.get(`https://kvhgeronimo.herokuapp.com/library/${this.props.itemID}`)
            .then(res => {
    ...
    addReport = () => {
        axios.post(`https://kvhgeronimo.herokuapp.com/library/update-library/${this.props.itemID}`, { name: this.state.name, text: this.state.text, links: this.state.links })
            .then(res => {
            ...
            <Modal size="lg" show={this.state.popup} onHide={this.handleOnHide}>
                ...
                <Modal.Body style={{ whiteSpace: "pre-line" }}>
                    <Container>
                        ...
                        <Button variant="dark" style={{ width: "100%" }} onClick={this.addReport}>Update Post</Button>
                    </Container>
                </Modal.Body>
            </Modal>
        )
